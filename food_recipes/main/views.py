    ...
    if request.method == "POST":
        ...
        if title:
            ...
            recipes_page, files, total = get_recipes_and_first_file(
                with_filter={"title": title}, page_number=page_number
            )

        elif my_rec:
            if request.user.id:
                recipes_page, files, total = get_recipes_and_first_file(
                    with_filter={"author": request.user.id},
...
def users_top(request):
    ...
    users_top = (
        User.objects.annotate(Count("recipe", distinct=True))
        .select_related("account")
        .order_by("-recipe__count")
    )
...
def stared(request, id):
    ...
    if not request.user or not request.user.id:
        ...
        user = Account.objects.get(user=request.user.id)
        liked = VotesConnection.objects.filter(recipe=rec1, user=user).first()
        if not liked:
...
def get_recipes_and_first_file(with_filter=False, page_number=0):
    if not with_filter:
        ...
        recipes = (
            Recipe.objects.filter(author=with_filter.get("author"))
    ...
    files = {
        int(file.id): [
            file.images.first().file if file.images.first() else None
        ]
...
class RecipeUpdateView(LoginRequiredMixin, UpdateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            for i in range(
                int(request.POST["images-TOTAL_FORMS"])
            ):  # удаление всех по галочкам
                field_delete = f"images-{i}-DELETE"
                field_image_id = f"images-{i}-id"
                if (
                    field_delete in request.POST
                    and request.POST[field_delete] == "on"
                ):
            ...
            for i in range(
                ...
                field_replace = (
                    f"images-{i}-file"  # должен быть в request.FILES
                )
                field_image_id = f"images-{i}-id"  # этот файл мы заменим
                if (
                    field_replace in request.FILES
                    and request.POST[field_image_id] != ""
                    and field_image_id not in deleted_ids
                ):
                    image = File.objects.get(
                        id=request.POST[field_image_id]
                    )  #
                    image.delete()  # удаляем старый файл
                    for img in request.FILES.getlist(
                        field_replace
                    ):  # новый добавили
...
class RecipeDeleteView(LoginRequiredMixin, View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        messages.add_message(
            self.request,
            messages.WARNING,
            f"Рецепт '{recipe.first().title}' удален",
        )
        recipe.first().delete()
        return HttpResponse("success")
